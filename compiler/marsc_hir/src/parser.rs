...
fn parse_ident(pair: Pair<Rule>) -> Result<Identifier> {
    let span = pair.as_span();
    Ok(Identifier { ident: pair.as_span().as_str(), span, })
}
...
fn parse_stmt(pair: Pair<Rule>) -> Result<Stmt> {
    ...
    match pair.as_rule() {
        ...
        Rule::r#break => Ok(Stmt::Break { span, }),
...
fn parse_additive_expr(pair: Pair<Rule>) -> Result<MathExpr> {
    ...
    let mut res = MathExpr::Additive { left: Box::new(left), right: Box::new(right), op, span };

    while !operations.is_empty() {
        left = res;
        right = numbers.pop().unwrap();
        op = operations.pop().unwrap();
        res = MathExpr::Additive { left: Box::new(left), right: Box::new(right), op, span };
    }
...
fn parse_multiplicative_expr(pair: Pair<Rule>) -> Result<MathExpr> {
    ...
    let mut right = numbers.pop().unwrap();
    let mut op = operations.pop().unwrap();
    let mut res = MathExpr::Multiplicative { left: Box::new(left), right: Box::new(right), op, span };

    while !operations.is_empty() {
        left = res;
        right = numbers.pop().unwrap();
        op = operations.pop().unwrap();
        res = MathExpr::Multiplicative { left: Box::new(left), right: Box::new(right), op, span };
    }
...
fn parse_power_expr(pair: Pair<Rule>) -> Result<MathExpr> {
    ...
    let mut res = MathExpr::Power { base: Box::new(base), exp: Box::new(exp), span };

    while !numbers.is_empty() {
        base = res;
        exp = numbers.pop().unwrap();
        res = MathExpr::Power { base: Box::new(base), exp: Box::new(exp), span };
    }
...
fn parse_struct_init_arg(pair: Pair<Rule>) -> Result<StructFieldDecl> {
    ...
    Ok(StructFieldDecl { ident, expr, span })
}
